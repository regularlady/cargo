...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Checking all available actions for preconditions match on this level
    ...
    def add_literal_level(self, level):
        ...
        # Checking all previous level actions and their effect
        for anode in self.a_levels[level-1]:
            anode_eff = anode.effnodes
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        #Confirm that the symbol matches but the positivity does not
        ...
        if not(set(node_s1.parents)&set(node_s2.parents)):
            ...
            for a in node_s1.parents:
                for am in node_s2.parents:
                    if a.is_mutex(am):
                        return True
        # no actions that could achieve the two literals at the same time
        return False
